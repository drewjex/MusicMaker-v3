...
class RepeatRhythms extends Maker implements IMaker {
            ...
            foreach ($pattern as $key => $value) {
                ...
                for ($i=0; $i<$value->num_notes; $i++) {
                    ...
                    if ($i == 0) {
                        ...
                        $piece = $music_changer->addClosestNoteFromSackInPieceAt($piece, $value->note_sack, $piece[$i-1]->note[count($piece[$i-1]->note)-1], $time, $increment);
                    }
                }
                $part = new Part();
                $measures = array();
